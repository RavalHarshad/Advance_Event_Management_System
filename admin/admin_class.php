...
Class Action {
	...
	function save_book(){
		...
		// Keep the existing end_duration if not provided
		if (!empty($end_duration)) {
			$data .= ", end_duration = '$end_duration' ";
		}
		...
		if(empty($id)) {
			// **Check for booking conflicts only on insert**
			$check = $this->db->query("SELECT * FROM venue_booking WHERE venue_id = '$venue_id' 
				AND ('$schedule' BETWEEN datetime AND end_duration 
				OR '$end_duration' BETWEEN datetime AND end_duration)");
	
			if ($check->num_rows > 0) {
				return json_encode(["status" => "error", "message" => "This hall is already booked for the selected date and time."]);
			}
	
			// Insert new booking
			$save = $this->db->query("INSERT INTO venue_booking SET ".$data);
		} else {
			// **Update booking without conflict check**
		...
		if($save) {
			...
			return json_encode(["status" => "error", "message" => "Booking failed. Please try again."]);
		}
	}
...
function manage_booking(){
    ...
    if (!isset($_POST['id']) || empty($_POST['id'])) {
        die(json_encode(["status" => "error", "message" => "Booking ID is missing!"]));
    }

    $id = $_POST['id'];
    $status = $_POST['status'];

    // Debugging: Print received values (Remove after testing)
    error_log("Updating Booking: ID=$id, Status=$status");

    // Update only the status field
    $sql = "UPDATE venue_booking SET status = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $status, $id); // Bind status (int) and id (int)

    if ($stmt->execute()) {
        return json_encode(["status" => "success", "message" => "Booking status updated successfully."]);
    } else {
        die(json_encode(["status" => "error", "message" => "SQL Error: " . $stmt->error . " | Query: " . $sql]));
    }
